...
**边缘吸附模式**默认使用三等分：拖到左/右边缘分别吸附为左三分之一/右三分之一，配合底部中间可以拼出左中右三等分布局；菜单中可切换为"左/右半屏"模式。

所有动作也可以在菜单栏 → 窗口分屏操作 中点击执行。菜单栏 → **自定义分屏快捷键…** 可以为每个动作录制自己的键位（要求至少包含 `Control` / `Option` / `Command` 之一；`Esc` 取消录制，可随时清除恢复默认）。窗口分屏基于 macOS 辅助功能 API 移动和缩放前台窗口，与 Rectangle 的窗口动作使用同一套机制；键位选用 `Ctrl + Option` 和弦，避免占用 macOS 文本编辑（`Command + 方向键`）和浏览器切标签（`Option + Command + 方向键`）等系统快捷键。
