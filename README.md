...
- 可选开启窗口分屏：`Ctrl + Option + 方向键` 把窗口吸附到左/右半屏、最大化或下半个屏幕；分屏后继续按方向键可切换到四分之一屏，再按一次可还原（Windows 风格连续吸附）。`Ctrl + Option + C` 居中，`Ctrl + Option + Delete` 还原。
...
分屏基于 macOS 辅助功能 API 移动和缩放前台窗口，与 Rectangle 的窗口动作使用同一套机制；键位选用 `Ctrl + Option` 和弦，避免占用 macOS 文本编辑（`Command + 方向键`）和浏览器切标签（`Option + Command + 方向键`）等系统快捷键。
